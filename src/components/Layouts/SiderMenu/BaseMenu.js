...
export default class BaseMenu extends PureComponent {
  ...
  getPath(item) {
    ...
    if (item.children && item.children.length) {
      path = this.getPath(item.children[0]);
    }
    return path;
  }
  ...
  getMenuItemPath = item => {
    ...
    if (item.children) {
      itemPath = this.getPath(item.children[0]);
    }
  ...
  handleClick = e => {
    const { menuData, clickNavigationMenu } = this.props;
    const viceMenuData = menuData.filter(item => item.path === e.key);
