        ...
        <Row type="flex" gutter={16}>
          <Col span={8}>
            <Form.Item label="门店名称">
              {getFieldDecorator('shopName')(<Input placeholder="请输入门店名称" allowClear />)}
            </Form.Item>
          </Col>
          <Col span={8}>
            <Form.Item>
...
class DMSelectStore extends Component {
  ...
  constructor(props) {
    ...
    this.columns = [
      { dataIndex: 'shopName', title: '门店名称', width: '30%' },
      // { dataIndex: 'shopOwner', title: '店长' },
  ...
  reload() {
    ...
    if (
      that.props.checkedshopList &&
      that.props.checkedshopList.length > 0 &&
      !selectedList.length
    ) {
      this.setState({
    ...
    if (
      that.props.checkedList != undefined &&
      that.props.checkedList.length > 0 &&
      !checkedList.length
    ) {
      this.setState({
  ...
  requestPageList(currentPage = 1, pageSize = 10, params) {
    ...
    if (
      params !== undefined &&
      params.shopGroupId === undefined &&
      (params.shopName === undefined || params.shopName === '')
    ) {
      this.setState({
  ...
  requestGroupList() {
    ...
    request('g2/wx.authority.group.shop.list', {
      ...
      .then(response => {
        ...
        that.setState({ dataList: [...response.list] });
      })
      .catch(() => {});
  }

  show = (store, completestore) => {
  ...
  handleCrowdClose = item => {
    ...
    const { checkedList, selectedList } = this.state;
    const index = checkedList.findIndex(v => v == item.id);
      ...
      rowSelection = {
        ...
        onChange: (selectedRowKeys, selectedRows) => {
          if (checkedList.length) {
            list.forEach(item => {
              const index = checkedList.findIndex(i => i === item.id);
              if (index !== -1) {
                checkedList.splice(index, 1);
            ...
            selectedRows.forEach(v => {
              const index = checkedList.findIndex(i => i === v.id);
              if (index === -1) {
    ...
    return (
      ...
      >
        ...
        <div className={styles.groupName}>
          <div className={styles.groupList} style={{ width: '15%' }}>
            <ul>
              <li
                className={currentGroup == null ? `${styles.active}` : ''}
              ...
              {dataList.map(item => {
                return (
                  <li
                    className={item.id == currentGroup ? `${styles.active}` : ''}
                    onClick={this.clickItem.bind(this, item)}
                  >
                    <Tooltip placement="top" title={item.groupName}>
                      {`${item.groupName.substring(0, 8)}...`}
                    </Tooltip>
                  </li>
                );
              })}
